...
async function updateQuestions() {
  try {
    ...
    // Find the Input/Output Practice topic
    const topic = await Topic.findOne({ title: 'Input/Output Practice' })
    if (!topic) {
      console.error('Topic "Input/Output Practice" not found')
      return
    }
    ...
    for (const questionData of questions) {
      const question = new Question({
        ...questionData,
        topicId: topic._id,
        createdBy: new mongoose.Types.ObjectId('000000000000000000000000') // Dummy admin ID
      })
      await question.save()
